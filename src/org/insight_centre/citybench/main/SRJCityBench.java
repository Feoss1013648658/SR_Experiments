...
import org.insight_centre.aceis.io.streams.csparql.SRJCSPARQLResultObserver;
import org.insight_centre.aceis.observations.SensorObservation;
import org.insight_centre.aceis.utils.test.PerformanceMonitor;
import org.insight_centre.citybench.reasoning.StaticTSReasonerLubmStream;
import org.insight_centre.citybench.reasoning.StaticTripleOutputStream;
import org.insight_centre.citybench.reasoning.TSReasoner;
import org.insight_centre.citybench.reasoning.TSReasonerAarhusParkingStream;
import org.insight_centre.citybench.reasoning.TSReasonerAarhusPollutionStream;
import org.insight_centre.citybench.reasoning.TSReasonerAarhusTrafficStream;
import org.insight_centre.citybench.reasoning.TSReasonerAarhusWeatherStream;
import org.insight_centre.citybench.reasoning.TSReasonerFmaStream;
import org.insight_centre.citybench.reasoning.TSReasonerLocationStream;
import org.insight_centre.citybench.reasoning.TSReasonerLubmStream;
import org.insight_centre.citybench.reasoning.TSReasonerOboStream;
import org.insight_centre.citybench.reasoning.TSReasonerSensorStream;
import org.insight_centre.citybench.reasoning.TripleOutputStream;
//import org.insight_centre.aceis.io.streams.csparql.CSPARQLResultObserver;
...
import sr.core.atom_based_reasoner.Window;
import sr.core.atom_based_reasoner.WindowInfo;

import java.nio.file.Files;
import java.nio.file.Paths;

public class SRJCityBench {
	public enum RSPEngine {
		cqels, csparql, tsreasoner, ptsreasoner;
	}

	public static ExecContext cqelsContext, tempContext;
	public static CsparqlEngineImpl csparqlEngine;
	public static TSReasoner tsReasoner;
	private static final Logger logger = LoggerFactory.getLogger(SRJCityBench.class);
	...
	public static void main(String[] args) {
		try {
			...
			SRJCityBench cb = new SRJCityBench(prop, parameters);
			cb.startTest();
	...
	private String dataset, ontology, cqels_query, csparql_query, streams, ruleset;
	 ...
	 * @throws Exception
	 */
	public SRJCityBench(Properties prop, HashMap<String, String> parameters) throws Exception {
		// parse configuration file
		// this.parameters = parameters;
		try {
			this.ruleset = prop.getProperty("ruleset");
		...
		if (parameters.containsKey("engine")) {
			...
			else if(parameters.get("engine").equals("tsreasoner"))
				this.engine  =RSPEngine.tsreasoner;
			else if(parameters.get("engine").equals("ptsreasoner"))
				this.engine = RSPEngine.ptsreasoner;
			else
...
//				+ this.queryDuplicates + ",e=" + this.engine + ",q=" + this.queries;// +
		...
		this.resultName = "q=" + this.queries + " r=" + this.rate + ",f=" + this.frequency + ",dup="
				+ this.queryDuplicates + ",e=" + this.engine;
		// parameters.toString();
		// initialize datasets
		try {
			
//			if(!this.engine.equals(RSPEngine.tsreasoner)){
...
//			}
	...
	private String initTSReasoner(){
		logger.info("init tsreasoner.....");
		if(this.engine == RSPEngine.tsreasoner){
			this.tsReasoner = new TSReasoner(false);
		}else{
			this.tsReasoner = new TSReasoner(true);
		}
		
		
		
		String ruleFile = this.ruleset + File.separator;
		String qid = null;
		for (Entry en : this.queryMap.entrySet()) {
			qid = en.getKey() + "-" + UUID.randomUUID();
			ruleFile = ruleFile + en.getKey().toString() + ".txt";
		}
		
		logger.info("ruleFile : " +  ruleFile);
		
		//register rules
		this.tsReasoner.registerRuleSet(ruleFile);
			
		
		return qid;
	}
	
	private void startTSReasoner(String qid) {
//		if(qid.contains("lubm"))
//			this.tsReasoner.getReasoner().setEval(true);
//		
		//register output stream
		if(duration == 0){
			this.tsReasoner.registerOutputStream(new  StaticTripleOutputStream(qid));
		}else{
			logger.info("qid" + qid);
			this.tsReasoner.registerOutputStream(new  TripleOutputStream(qid));
		}
			
		//register input streams
		try {
			startTSReasonerStreamsFromQuery();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}			
	...
	private void registerCSPARQLQuery(String qid, String query) throws ParseException {
		if (!registeredQueries.keySet().contains(qid)) {
			...
			SRJCSPARQLResultObserver cro = new SRJCSPARQLResultObserver(qid);
	...
	private void startTSReasonerStreamsFromQuery() throws Exception {
		Map<String,WindowInfo> sw = this.tsReasoner.getReasoner().getRuleSet().getStreamWindowMap();
		for(Entry<String, WindowInfo> entry : sw.entrySet()){
			String sn = entry.getKey();
			String uri = sn;
			String path = this.streams + "/" + sn.split("#")[1] + ".stream";
			logger.info("sn = " + sn);
			logger.info("uri = " + uri);
			logger.info("path = " + path);
			if (!this.startedStreams.contains(uri)) {
				this.startedStreams.add(uri);
				TSReasonerSensorStream css;
				if(!uri.contains("instance") &&!uri.contains("lubm")){
					EventDeclaration ed = er.getEds().get(uri);
					if (ed.getEventType().contains("traffic")) {
						css = new TSReasonerAarhusTrafficStream(uri, path, ed, start, end);
					} else if (ed.getEventType().contains("pollution")) {
						css = new TSReasonerAarhusPollutionStream(uri, path, ed, start, end);
					} else if (ed.getEventType().contains("weather")) {
						css = new TSReasonerAarhusWeatherStream(uri, path, ed, start, end);
					} else if (ed.getEventType().contains("location"))
						css = new TSReasonerLocationStream(uri, path, ed);
					else if (ed.getEventType().contains("parking"))
						css = new TSReasonerAarhusParkingStream(uri, path, ed, start, end);
					...
					if(uri.contains("fma")){
						css = new TSReasonerFmaStream(uri, path, new Double(this.rate).intValue());
					}else if(uri.contains("obo")){
						css = new TSReasonerOboStream(uri, path, new Double(this.rate).intValue());
					}else if(uri.contains("lubm")){
						if(duration !=0)
							css = new TSReasonerLubmStream(uri, path, new Double(this.rate).intValue());
						else
							css = new StaticTSReasonerLubmStream(uri, path, new Double(this.rate).intValue());
					}
					else
						throw new Exception("Sensor type not supported.");
				...
				css.setRate(this.rate);
				css.setFreq(this.frequency);
				this.tsReasoner.getReasoner().registerStream(css, entry.getValue());
				new Thread(css).start();
				startedStreamObjects.add(css);
			}
		}
	...
	protected void startTest() throws Exception {
		...
		else if(this.engine == RSPEngine.tsreasoner || this.engine == RSPEngine.ptsreasoner){
			String qid = this.initTSReasoner();
			pm = new PerformanceMonitor(queryMap, duration, queryDuplicates, resultName);		
			new Thread(pm).start();
			this.startTSReasoner(qid);
		}
